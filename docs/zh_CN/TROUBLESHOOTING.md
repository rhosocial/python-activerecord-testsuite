...
本文档记录了在运行测试套件时可能遇到的一些常见问题、错误及其解决方案。这些经验来自于一次完整的调试过程，旨在帮助后端开发者快速定位和解决问题。
...
2.  **`AttributeError: 'dict' object has no attribute 'connection_config'`**
    *   **问题**：在修复上一个问题后，我们尝试 `User.configure(config=db_session.connection_config, ...)`。但这导致了新错误，表明传递给模型夹具的 `database` 参数是一个字典，而不是预期的 `backend` 实例。
    *   **分析**：对 `pytest` 的参数化机制理解有误。未使用 `indirect=True` 导致夹具获得的是参数本身，而非夹具的执行结果。
...
*   **原因**：尽管 `teardown_database` 在每个测试后被调用，但其中的 `DROP TABLE` 语句没有被正确地提交到数据库。这是因为 `ActiveRecord` 库的 `execute` 方法默认只对 DML 语句 (如 `INSERT`、`UPDATE`) 自动提交事务，而 `DROP TABLE` 是 DDL 语句，未被提交。
