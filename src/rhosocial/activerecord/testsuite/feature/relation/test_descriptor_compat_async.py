...
"""
Tests for async/async descriptor compatibility.

Async descriptors (BelongsTo, HasMany, HasOne) must not be used on async models
(AsyncActiveRecord), and async descriptors (AsyncBelongsTo, AsyncHasMany, AsyncHasOne)
must not be used on async models (ActiveRecord). Mixing them raises TypeError
...
class TestAsyncDescriptorOnAsyncModel:
    """Async descriptors on async models raise TypeError at class creation."""

    @pytest.mark.asyncio

    async def test_sync_belongs_to_on_async_model_raises(self):
        """Async BelongsTo on AsyncActiveRecord raises TypeError."""
        from rhosocial.activerecord.model import AsyncActiveRecord
        from rhosocial.activerecord.relation.descriptors import BelongsTo

        with pytest.raises(TypeError, match="Async relation descriptor.*async model"):
    ...
    async def test_sync_has_many_on_async_model_raises(self):
        """Async HasMany on AsyncActiveRecord raises TypeError."""
        from rhosocial.activerecord.model import AsyncActiveRecord
    ...
    async def test_sync_has_one_on_async_model_raises(self):
        """Async HasOne on AsyncActiveRecord raises TypeError."""
        from rhosocial.activerecord.model import AsyncActiveRecord
