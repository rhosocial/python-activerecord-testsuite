...
class TestSyncPydanticNativeValidation:
    ...
    def test_field_constraints_fail_on_init(self, pydantic_validated_model):
        """Pydantic field constraints (regex, ge, gt, multiple_of, length) must reject invalid values on init."""
    ...
    def test_field_validator_transforms_value(self, pydantic_validated_model):
        """A field_validator should transform input and the result should round-trip to the database."""
        ...
        assert model.save() == 1, "Expected save() to affect 1 row"
    ...
    def test_validation_runs_again_on_save_after_assignment(self, pydantic_validated_model):
        """Field constraints should be re-validated on save() after assignment."""
        model = pydantic_validated_model(**valid_pydantic_data())
        model.save()
    ...
    def test_model_validator_runs_again_on_save_after_assignment(self, pydantic_validated_model):
        """Cross-field model_validator should be re-validated on save() after assignment."""
        model = pydantic_validated_model(**valid_pydantic_data())
        model.save()
    ...
    def test_pydantic_coercion_persists_and_loads(self, pydantic_validated_model):
        """String inputs should be coerced to int/Decimal/datetime and round-trip to typed values."""
        data = valid_pydantic_data()
    ...
    def test_field_metadata_is_preserved(self, pydantic_validated_model):
        """Field metadata (title, description, json_schema_extra) should survive into the JSON schema."""
    ...
    def test_field_proxy_queries_pydantic_fields(self, pydantic_validated_model):
        """FieldProxy (model.c.<name>) should support equality and comparison queries on Pydantic fields."""
        model = pydantic_validated_model(**valid_pydantic_data())
        model.save()
    ...
    def test_save_succeeds_after_fixing_invalid_assignment(self, pydantic_validated_model):
        """A successful assignment after a failed one should save cleanly."""
        model = pydantic_validated_model(**valid_pydantic_data())
        model.save()
        ...
        model.quantity = 8
        assert model.save() == 1, "Expected the corrected save() to affect 1 row"
    ...
    def test_mutable_default_factory_values_are_instance_isolated(self):
        """Mutable default_factory values should not be shared between instances."""
    ...
    def test_lifecycle_hooks_and_unset_none_are_preserved(self):
        """model_post_init and model_validator should run; unset vs explicit None should be tracked."""
    ...
    def test_computed_field_and_serializer_are_dump_only_boundaries(self):
        """computed_field and field_serializer should shape model_dump() output."""
    ...
    def test_annotated_field_and_use_column_metadata_coexist(self):
        """Annotated Field() and UseColumn() metadata should coexist on the same field."""
        model = PydanticV2BoundaryModel(
            first_name="Ada",
            last_name="Lovelace",
            score=Decimal("9.5"),
        ...
        assert PydanticV2BoundaryModel.get_column_name("annotated_code") == "annotated_code_col", \
    ...
    def test_aliases_are_independent_from_column_mapping(self):
        """Field aliases and UseColumn mappings should operate independently."""
        ...
        assert AliasBoundaryModel.get_column_name("external_id") == "external_id_col", \
            "Expected get_column_name to reflect UseColumn"

    def test_extra_config_modes_preserve_pydantic_behavior(self):
        """Extra config modes (forbid/ignore/allow) should preserve Pydantic semantics."""
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_strict_fields_reject_coercion(self):
        """Strict-mode fields should reject type coercion of inputs."""
    ...
    def test_validate_default_runs_on_model_construction(self):
        """Fields with validate_default=True should run validators on default values."""
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_validator_pipeline_modes_are_preserved(self):
        """before/after/wrap validator modes should each transform their inputs."""
    ...
    def test_json_enum_container_and_nested_model_contracts(self):
        """JSON serialization should preserve enums, datetimes, nested models, and containers."""
    ...
    def test_from_attributes_constructs_from_plain_objects(self):
        """from_attributes mode should construct models from arbitrary objects with matching attributes."""
        class SourceObject:
            name = "Ada"
            quantity = 3
    ...
    def test_annotated_metadata_order_is_not_significant(self):
        """The order of Annotated metadata (Field then UseColumn vs UseColumn then Field) should be irrelevant."""
        ...
        assert AnnotatedMetadataOrderModel.get_column_name("field_first") == "field_first_col", \
            "Expected field_first to map to field_first_col"
        assert AnnotatedMetadataOrderModel.get_column_name("column_first") == "column_first_col", \
    ...
    def test_alias_use_column_conflicts_keep_separate_meanings(self):
        """Conflicting alias and UseColumn values should remain distinct surface concepts."""
        ...
        assert AliasUseColumnConflictModel.get_column_name("same_name") == "externalName", \
            "Expected same_name to map to externalName column"
        assert AliasUseColumnConflictModel.get_column_name("different_name") == "db_different_name", \
    ...
    def test_validate_assignment_updates_dirty_tracking_only_after_success(self):
        """validate_assignment should only mark fields dirty after the value is accepted."""
