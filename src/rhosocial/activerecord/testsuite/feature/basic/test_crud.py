...
class TestAsyncCRUD:
    ...
    async def test_refresh_record(self, async_validated_user_class):
        ...
        # Update data with another instance
        another_instance = await async_validated_user_class.find_one(user.id)
        another_instance.username = 'refreshed_user'
        await another_instance.save()
    ...
    async def test_query_methods(self, async_validated_user_class):
        ...
        # avg_age = await async_validated_user_class.query().select('AVG(age) as avg_age').one()  # TODO: Aggregate queries not supported yet, to be improved in the future.
        # assert avg_age['avg_age'] == 31  # 30 + 31 + 32 / 3
