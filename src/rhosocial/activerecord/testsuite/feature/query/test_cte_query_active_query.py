...
class TestCTEQueryActiveQuery:
    ...
    def test_single_active_query_cte(self, tree_fixtures):
        ...
        # Pass a string query as the main query to select from the CTE
        cte_query.query("SELECT high_value_nodes.name, high_value_nodes.value FROM high_value_nodes")
        
        # Execute the CTE query
        result = cte_query.aggregate()
    ...
    def test_multiple_active_query_cte(self, tree_fixtures):
        ...
        # Pass a string query as the main query to select from both CTEs
        cte_query.query("SELECT high_values.name, high_values.value, low_values.name as low_name, low_values.value as low_value FROM high_values LEFT JOIN low_values ON high_values.id = low_values.id")
        
        # Execute the CTE query
        result = cte_query.aggregate()
        
        assert len(result) >= 2  # Should have results from both CTEs
...
class TestAsyncCTEQueryActiveQuery:
    ...
    async def test_single_active_query_cte(self, async_tree_fixtures):
        ...
        # Execute the CTE query
        result = await cte_query.aggregate()
