...
"""
Fixture definitions for WorkerPool testing - Query Feature.
...
def _get_worker_tasks_module_path() -> str:
    ...
    Reads from environment variable TESTSUITE_QUERY_WORKER_TASKS_MODULE,
    defaults to 'providers.query_worker_tasks'.
...
def worker_tasks():
    ...
    Returns:
        Module object containing various task functions
    """
...
@pytest.fixture(scope="function")
def query_worker_connection_params(order_fixtures) -> Dict[str, Any]:
    """
    Extract serializable connection parameters from configured models.

    The environment is already prepared by the order_fixtures fixture,
    this just extracts connection information for Worker use.

    Args:
        order_fixtures: Tuple of configured model classes (User, Order, OrderItem)

    Returns:
        dict: Serializable connection parameters
