...
class TestParallelQueries:
    ...
    def test_parallel_count_queries(self, order_fixtures_for_worker):
        """Test parallel execution of count queries."""
        ...
        # All counts should return the same value
        assert len(set(results)) == 1

    def test_parallel_order_queries_by_user(self, order_fixtures_for_worker):
        """Test parallel queries for orders by user."""
        User, Order, OrderItem = order_fixtures_for_worker['models']
        conn_params = order_fixtures_for_worker['conn_params']
        ...
        with WorkerPool(n_workers=4) as pool:
            futures = [
                pool.submit(query_orders_by_user_task, uid, conn_params)
                for uid in user_ids
            ]
            results = [f.result(timeout=60) for f in futures]
    ...
    def test_parallel_aggregate_queries(self, order_fixtures_for_worker):
        """Test parallel aggregate queries."""
        User, Order, OrderItem = order_fixtures_for_worker['models']
        conn_params = order_fixtures_for_worker['conn_params']
        ...
        with WorkerPool(n_workers=4) as pool:
            # Execute multiple aggregate queries in parallel
            futures = [
                pool.submit(aggregate_query_task, conn_params)
                for _ in range(5)
            ]
            results = [f.result(timeout=60) for f in futures]

        # All aggregate results should be consistent
        totals = {r['total'] for r in results}
        counts = {r['count'] for r in results}
        assert len(totals) == 1
        assert len(counts) == 1

    def test_parallel_order_item_queries(self, order_fixtures_for_worker):
        """Test parallel queries for order items."""
        User, Order, OrderItem = order_fixtures_for_worker['models']
        conn_params = order_fixtures_for_worker['conn_params']
        ...
        with WorkerPool(n_workers=4) as pool:
            futures = [
                pool.submit(query_order_items_task, oid, conn_params)
