...
async def test_common_sql_standard_features(async_order_fixtures):
    ...
    # 2. WHERE condition
    active_orders = AsyncOrder.query().where(AsyncOrder.c.status == 'active').all()
    assert len(active_orders) == 3  # 0, 2, 4 index orders are active

    # 3. ORDER BY (using correct syntax)
    ordered_orders = AsyncOrder.query().order_by(AsyncOrder.c.total_amount).all()
    ...
    ordered_desc_orders = AsyncOrder.query().order_by((AsyncOrder.c.total_amount, "DESC")).all()
    assert ordered_desc_orders[0].total_amount >= ordered_desc_orders[-1].total_amount

    # 4. LIMIT and OFFSET
    limited_orders = AsyncOrder.query().order_by(AsyncOrder.c.order_number).limit(2).offset(1).all()
    ...
    # 6. SUM aggregation
    total_amount = AsyncOrder.query().sum_(AsyncOrder.c.total_amount)
    ...
    grouped_results = AsyncOrder.query() \
        .select(AsyncOrder.c.status, await functions.count(dialect, '*').as_('count')) \
...
async def test_aggregation_compatibility(async_order_fixtures):
    ...
    average = AsyncOrder.query().avg(AsyncOrder.c.total_amount)
...
async def test_join_compatibility(async_blog_fixtures):
    ...
    user_with_posts = AsyncUser.query().where(AsyncUser.c.id == user.id).all()
    assert len(user_with_posts) == 1
    
    post_for_user = AsyncPost.query().where(AsyncPost.c.user_id == user.id).all()
    ...
    try:
        ...
        joined_results = AsyncPost.query() \
