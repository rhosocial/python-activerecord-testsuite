...
def test_in_list_with_values(order_fixtures):
    ...
    user = User(username='in_list_user', email='inlist@example.com', age=30)
    user.save()
    ...
    results = Order.query().in_list(Order.c.status, ['pending', 'shipped', 'delivered']).all()
...
def test_in_list_empty_result_true(order_fixtures):
    """
    ...
    user = User(username='empty_in_user', email='empty@example.com', age=30)
    user.save()
    ...
    results = Order.query().in_list(Order.c.status, [], empty_result=True).all()
    assert len(results) == 0


def test_in_list_empty_result_false(order_fixtures):
    """
    ...
    user = User(username='no_empty_in_user', email='noempty@example.com', age=30)
    user.save()
    ...
    results = Order.query().in_list(Order.c.status, [], empty_result=False).all()
...
def test_not_in_with_values(order_fixtures):
    ...
    user = User(username='not_in_user', email='notin@example.com', age=30)
    user.save()
    ...
    results = Order.query().not_in(Order.c.status, ['pending', 'cancelled']).all()
...
def test_not_in_empty_behavior(order_fixtures):
    """
    ...
    user = User(username='not_empty_in_user', email='notempty@example.com', age=30)
    user.save()
    ...
    results = Order.query().not_in(Order.c.status, [], empty_result=False).all()
    assert len(results) == 3

    # Empty list NOT IN query with empty_result=True, should return empty results
    results = Order.query().not_in(Order.c.status, [], empty_result=True).all()
    assert len(results) == 0


def test_between_operation(order_fixtures):
    """
    ...
    user = User(username='between_user', email='between@example.com', age=30)
    user.save()
    ...
    results = Order.query() \
        .where((Order.c.total_amount >= Decimal('100.00')) & (Order.c.total_amount <= Decimal('200.00'))) \
        .all()
...
def test_not_between_operation(order_fixtures):
    """
    ...
    user = User(username='not_between_user', email='notbetween@example.com', age=30)
    user.save()
...
def test_comparison_operators(order_fixtures):
    """
    ...
    user = User(username='comp_user', email='comp@example.com', age=30)
    user.save()
