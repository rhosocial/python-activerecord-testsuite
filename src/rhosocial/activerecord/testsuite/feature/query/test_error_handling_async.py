...
async def test_invalid_parameter_handling(async_order_fixtures):
    ...
    user = AsyncUser(username='error_param_user', email='errorparam@example.com', age=30)
    await user.save()

    order = AsyncOrder(user_id=user.id, order_number='ERR-001', total_amount=Decimal('100.00'))
    await order.save()
...
async def test_type_error_handling(async_order_fixtures):
    ...
    user = AsyncUser(username='type_error_user', email='typeerror@example.com', age=30)
    await user.save()

    order = AsyncOrder(user_id=user.id, order_number='TYPE-001', total_amount=Decimal('150.00'))
    await order.save()
...
async def test_null_value_handling(async_order_fixtures):
    ...
    user = AsyncUser(username='null_user', email='null@example.com', age=None)  # Age is null
    await user.save()
...
async def test_sql_injection_protection(async_order_fixtures):
    ...
    user = AsyncUser(username='inject_user', email='inject@example.com', age=30)
    await user.save()

    order = AsyncOrder(user_id=user.id, order_number='INJECT-001', total_amount=Decimal('100.00'))
    await order.save()
...
async def test_parameterized_query_validation(async_order_fixtures):
    ...
    user = AsyncUser(username='param_user', email='param@example.com', age=30)
    await user.save()
...
async def test_dangerous_character_escaping(async_order_fixtures):
    ...
    user = AsyncUser(username='escape_user', email='escape@example.com', age=30)
    await user.save()

    # AsyncOrder number with potentially dangerous characters for escaping test
    dangerous_order_number = "Dangerous'AsyncOrder\"Name;DROP"
...
async def test_column_resolution_errors(async_order_fixtures):
    ...
    user = AsyncUser(username='col_res_user', email='colres@example.com', age=30)
    await user.save()

    order = AsyncOrder(user_id=user.id, order_number='COLRES-001', total_amount=Decimal('89.99'), status='pending')
    await order.save()
...
async def test_division_by_zero_handling(async_order_fixtures):
    ...
    user = AsyncUser(username='div_zero_user', email='divzero@example.com', age=30)
    await user.save()
...
async def test_invalid_sql_syntax_handling(async_order_fixtures):
    ...
    user = AsyncUser(username='invalid_sql_user', email='invalidsql@example.com', age=30)
    await user.save()
...
async def test_transaction_rollback_on_error(async_order_fixtures):
    ...
    user = AsyncUser(username='trans_rollback_user', email='transrollback@example.com', age=30)
    await user.save()

    # Test transaction behavior with error
    try:
        # Start a transaction implicitly with save
        problematic_order = AsyncOrder(
            user_id=user.id,
...
async def test_escape_consistency_single_quote(async_order_fixtures):
    ...
    user = AsyncUser(username='esc_quote', email='esc_quote@example.com', age=30)
    await user.save()
...
async def test_escape_consistency_double_quote(async_order_fixtures):
    ...
    user = AsyncUser(username='esc_dquote', email='esc_dquote@example.com', age=30)
    await user.save()
...
async def test_escape_consistency_backslash(async_order_fixtures):
    ...
    user = AsyncUser(username='esc_bslash', email='esc_bslash@example.com', age=30)
    await user.save()
...
async def test_escape_consistency_sql_keywords(async_order_fixtures):
    ...
    user = AsyncUser(username='esc_keywords', email='esc_keywords@example.com', age=30)
    await user.save()
...
async def test_injection_payloads_as_data(async_order_fixtures):
    ...
    user = AsyncUser(username='inject_immunity', email='inject_immunity@example.com', age=30)
    await user.save()
...
async def test_sql_comment_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='comment_immune', email='comment_immune@example.com', age=30)
    await user.save()
...
async def test_special_character_full_matrix(async_order_fixtures):
    ...
    user = AsyncUser(username='matrix_user', email='matrix@example.com', age=30)
    await user.save()
...
async def test_value_equivalence_expression_vs_parameterized(async_order_fixtures):
    ...
    user = AsyncUser(username='equiv_user', email='equiv@example.com', age=30)
    await user.save()
...
async def test_like_no_auto_escape(async_order_fixtures):
    ...
    user = AsyncUser(username='like_noesc', email='like_noesc@example.com', age=30)
    await user.save()
...
async def test_null_comparison_with_is_null(async_order_fixtures):
    ...
    user_null = AsyncUser(username='is_null_user', email='isnull@example.com', age=None)
    await user_null.save()

    user_with_age = AsyncUser(username='has_age_user', email='hasage@example.com', age=25)
    await user_with_age.save()
...
async def test_in_clause_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='in_inject', email='in_inject@example.com', age=30)
    await user.save()
...
async def test_qmark_placeholder_escaping(async_order_fixtures):
    ...
    user = AsyncUser(username='qmark_esc', email='qmark_esc@example.com', age=30)
    await user.save()
...
async def test_tautology_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='tautology_test', email='tautology@example.com', age=30)
    await user.save()
...
async def test_query_error_recovery(async_order_fixtures):
    ...
    user = AsyncUser(username='recovery_user', email='recovery@example.com', age=30)
    await user.save()
...
async def test_second_order_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='second_order', email='second_order@example.com', age=30)
    await user.save()
...
async def test_unicode_normalization_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='unicode_inject', email='unicode_inject@example.com', age=30)
    await user.save()
...
async def test_case_variation_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='case_vary', email='case_vary@example.com', age=30)
    await user.save()
...
async def test_comment_style_variation_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='comment_var', email='comment_var@example.com', age=30)
    await user.save()
...
async def test_newline_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='newline_inj', email='newline_inj@example.com', age=30)
    await user.save()
...
async def test_null_byte_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='nullbyte_inj', email='nullbyte_inj@example.com', age=30)
    await user.save()
...
async def test_boolean_blind_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='bool_blind', email='bool_blind@example.com', age=30)
    await user.save()
...
async def test_dbms_specific_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='dbms_inject', email='dbms_inject@example.com', age=30)
    await user.save()
...
async def test_like_wildcard_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='like_inject', email='like_inject@example.com', age=30)
    await user.save()
...
async def test_heavy_query_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='heavy_inject', email='heavy_inject@example.com', age=30)
    await user.save()
...
async def test_nested_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='nested_inj', email='nested_inj@example.com', age=30)
    await user.save()
...
async def test_out_of_band_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='oob_inject', email='oob_inject@example.com', age=30)
    await user.save()
...
async def test_stacked_query_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='stacked_inj', email='stacked_inj@example.com', age=30)
    await user.save()
...
async def test_encoding_variation_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='encode_inj', email='encode_inj@example.com', age=30)
    await user.save()
...
async def test_error_based_injection_immunity(async_order_fixtures):
    ...
    user = AsyncUser(username='err_inject', email='err_inject@example.com', age=30)
    await user.save()
