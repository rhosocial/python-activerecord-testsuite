# src/rhosocial/activerecord/testsuite/feature/query/test_relations_basic_async.py
...
async def test_relation_param_types(async_order_fixtures):
    ...
    with pytest.raises(TypeError, match="foreign_key must be a string"):
        class InvalidOrder(ActiveRecord):
            user: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(foreign_key=123)
...
async def test_relation_inheritance(async_order_fixtures):
    ...
    class BaseOrder(ActiveRecord):
        ...
        __backend__ = AsyncOrder.__backend__
        user: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(
            foreign_key='user_id'
        )
...
async def test_relation_registration():
    ...
    class AsyncUser(ActiveRecord):
        __table_name__ = "users"
        id: Optional[int] = None
        username: str
        email: EmailStr

    class AsyncPost(ActiveRecord):
        __table_name__ = "posts"
        id: Optional[int] = None
        user_id: int
        title: str

        user: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(foreign_key='user_id')
...
async def test_relation_cache_management():
    ...
    post = AsyncPost(user_id=1, title="Test AsyncPost")
...
async def test_relation_descriptor_initialization():
    ...
    class AsyncPost(ActiveRecord):
        ...
        cached_user: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(
            foreign_key='user_id',
            cache_config=CacheConfig(ttl=60)
        )

    assert AsyncPost.get_relation('user').foreign_key == 'user_id'
...
async def test_belongs_to_relation():
    ...
    class AsyncPost(ActiveRecord):
        ...
        author: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(foreign_key='user_id')
...
async def test_has_many_relation():
    ...
    class AsyncComment(ActiveRecord):
    ...
    class AsyncPost(ActiveRecord):
        ...
        comments: ClassVar[HasMany['AsyncComment']] = HasMany(foreign_key='post_id')

    relation = AsyncPost.get_relation('comments')
...
async def test_has_one_relation():
    ...
    class AsyncProfile(ActiveRecord):
    ...
    class AsyncUser(ActiveRecord):
        ...
        profile: ClassVar[HasOne['AsyncProfile']] = HasOne(foreign_key='user_id')
...
async def test_multiple_relations_on_model():
    ...
    relations = AsyncPost.get_relations()
    assert 'author' in relations
    assert 'comments' in relations

    assert isinstance(AsyncPost.get_relation('author'), BelongsTo)
...
async def test_relation_inverse_of():
    ...
    class AsyncPost(ActiveRecord):
        ...
        author: ClassVar[BelongsTo['AsyncUser']] = BelongsTo(
            foreign_key='user_id',
            inverse_of='posts'
        )

    user_relation = AsyncPost.get_relation('author')
    post_relation = AsyncUser.get_relation('posts')
...
async def test_relation_query_method():
    ...
    assert hasattr(AsyncPost, 'author_query')
    assert callable(getattr(AsyncPost, 'author_query'))
