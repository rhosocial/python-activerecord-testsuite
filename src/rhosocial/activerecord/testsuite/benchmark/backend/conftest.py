"""Pytest fixtures for backend direct benchmark scenarios."""
...
PROVIDER_KEY = "benchmark.backend.IBackendBenchmarkProvider"
...
SCENARIO_PARAMS = _get_scenarios() or [
    pytest.param("default", marks=pytest.mark.skip(reason="No backend benchmark provider found"))
]


@pytest.fixture(scope="function", params=SCENARIO_PARAMS)
def backend_sync_context(request, benchmark_size):
    provider_class = _get_provider_class()
    if not provider_class:
        pytest.skip("No backend benchmark provider found")
    ...
    try:
        ...
        pytest.skip(str(exc))
    if context is None:
        pytest.skip(f"Backend sync benchmark is not supported for scenario {scenario!r}")
    yield context
    provider.teardown_benchmark_sync(scenario, context)


@pytest.fixture(scope="function", params=SCENARIO_PARAMS)
def backend_async_context(request, benchmark_size):
    provider_class = _get_provider_class()
    if not provider_class:
        pytest.skip("No backend benchmark provider found")
    provider = provider_class()
    ...
    try:
        try:
            ...
            pytest.skip(str(exc))
        if context is None:
            pytest.skip(f"Backend async benchmark is not supported for scenario {scenario!r}")
